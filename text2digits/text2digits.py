...
class Text2Digits(object):
    ...
    def convert(self, text: str) -> Tuple[str, Dict[str, str]]:
        """
    ...
    def _parse(self, tokens: List[Token]) -> Tuple[str, Dict[str, str]]:
        """
            ...
            while i < len(tokens):
                ...
                if tokens[i].type != WordType.OTHER:
                    ...
                    if n_match > 0:
                        ...
                        n_matched = tokens[i:i + n_match]
                        n_raw = [raw_token.word_raw for raw_token in n_matched]
                        joined_n_raw = " ".join(n_raw)
                        # print("Preprocessing token: ", tokens[i:i + n_match])

                        token = rule.action(n_matched)
                        new_tokens.append(token)
                        d[joined_n_raw] = token
                        i += n_match
                    else:
        ...
        for token in tokens:
            if token.is_ordinal() and not self.convert_ordinals:
                ...
                if token.is_ordinal() and self.add_ordinal_ending:
                    ...
                    d[token.word_raw] = token.text() + token.ordinal_ending
